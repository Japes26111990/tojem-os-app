...
import { X, CheckCircle2, DollarSign, Clock, Zap, Edit, Trash2, Save, XCircle, Award, RefreshCw, Lightbulb } from 'lucide-react'; // Added Lightbulb
import { processConsumables, calculateJobDuration } from '../../../utils/jobUtils';
import { useAuth } from '../../../contexts/AuthContext';
import { giveKudosToJob, updateDocument, updateStandardRecipe, addKaizenSuggestion } from '../../../api/firestore'; // Added addKaizenSuggestion
import ConsumableEditor from '/src/components/features/recipes/ConsumableEditor.jsx';
import KaizenSuggestionModal from './KaizenSuggestionModal'; // Import the new modal
...
const JobDetailsModal = ({ job, onClose, currentTime, employeeHourlyRates, overheadCostPerHour, allEmployees, onUpdateJob, onDeleteJob, allInventoryItems, allTools, allToolAccessories }) => {
    ...
    const [isKaizenModalOpen, setIsKaizenModalOpen] = useState(false); // State for the new modal
    ...
    return (
        <>
                        ...
                        {isEditing ? (
                            ...
                            <>
                                ...
                                <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-gray-700">
                                    {/* NEW BUTTON for Kaizen */}
                                    <Button onClick={() => setIsKaizenModalOpen(true)} variant="secondary" className="bg-yellow-600/20 hover:bg-yellow-600/40 text-yellow-300">
                                        <Lightbulb size={18} className="mr-2"/> Suggest Improvement
                                    </Button>
            ...
            {isKaizenModalOpen && (
                <KaizenSuggestionModal 
                    job={job}
                    user={user}
                    onClose={() => setIsKaizenModalOpen(false)}
                    onSubmit={addKaizenSuggestion}
                />
            )}
        </>
    );
